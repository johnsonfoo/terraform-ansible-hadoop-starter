...
resource "aws_instance" "hadoop_master_ec2_instance" {
  ...
  provisioner "remote-exec" {
    inline = [
      "cd $HADOOP_CONF_DIR",
      "sudo xmlstarlet ed -L -s /configuration -t elem -n property -s /configuration/property -t elem -n name -v fs.defaultFS -s /configuration/property -t elem -n value -v hdfs://${self.private_dns}:9000 core-site.xml",
      "sudo xmlstarlet ed -L core-site.xml",
      "sudo xmlstarlet ed -L -s /configuration -t elem -n property -s /configuration/property -t elem -n name -v yarn.nodemanager.aux-services -s /configuration/property -t elem -n value -v mapreduce_shuffle -s /configuration -t elem -n property -s /configuration/property[2] -t elem -n name -v yarn.resourcemanager.hostname -s /configuration/property[2] -t elem -n value -v ${self.private_dns} yarn-site.xml",
      "sudo xmlstarlet ed -L yarn-site.xml",
      "sudo cp mapred-site.xml.template mapred-site.xml",
      "sudo xmlstarlet ed -L -s /configuration -t elem -n property -s /configuration/property -t elem -n name -v mapreduce.jobtracker.address -s /configuration/property -t elem -n value -v ${self.private_dns}:54311 -s /configuration -t elem -n property -s /configuration/property[2] -t elem -n name -v mapreduce.framework.name -s /configuration/property[2] -t elem -n value -v yarn mapred-site.xml",
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo touch masters",
      "echo ${self.private_dns} | sudo tee -a masters",
      "sudo rm slaves",
